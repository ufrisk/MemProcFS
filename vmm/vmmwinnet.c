// vmmwinnet.c :  implementation of functionality related to the Windows networking.
//
...
*/
_Success_(return)
BOOL VmmWinTcpIp_TcpE_GetAddressEPs(_In_ PVMM_PROCESS pSystemProcess, _Inout_ POB_SET pObSet_TcpEndpoints)
{
    ...
    while((va = ObSet_Pop(pObTcpE))) {
        ...
        ObSet_Push(pObSet_TcpEndpoints, va + 0x10);
    }
    if(0 == ObSet_Size(pObSet_TcpEndpoints)) { goto fail; }
    fResult = TRUE;
fail:
    Ob_DECREF(pObTcHT);
    Ob_DECREF(pObHTab);
...
*/
_Success_(return)
BOOL VmmWinTcpIp_TcpE_Enumerate(_In_ PVMM_PROCESS pSystemProcess, _In_ POB_SET pSet_TcpE, _In_count_(cTcpEs) PVMMWIN_TCPIP_ENTRY pTcpEs, _In_ DWORD cTcpEs, _Out_ PDWORD pcTcpEs)
{
    BOOL f;
    QWORD va;
    ...
    if(cTcpEs < ObSet_Size(pSet_TcpE)) { goto fail; }
    ...
    while((va = ObSet_Pop(pSet_TcpE))) {
        ...
        ObSet_Push(pObPrefetch, pE->_Reserved_vaINET_AF - 0x10);
        ObSet_Push(pObPrefetch, pE->_Reserved_vaINET_Addr);
        c++;
    }
    ...
    for(i = 0; i < c; i++) {
        ...
        ObSet_Push(pObPrefetch, pE->_Reserved_vaINET_Src);
        ObSet_Push(pObPrefetch, pE->_Reserved_vaINET_Dst);
    }
...
BOOL VmmWinTcpIp_TcpE_Get(_Out_ PPVMMWIN_TCPIP_ENTRY ppTcpE, _Out_ PDWORD pcTcpE)
{
    ...
    VmmWinTcpIp_TcpE_Fuzz(pObSystemProcess, ObSet_Get(pObTcpE, 0));
    if(!ctxVmm->TcpIp.OTcpE._fValid) { goto fail; }
    cTcpEs = ObSet_Size(pObTcpE);
...
VOID VmmWinNet_CallbackCleanup_ObMapNet(PVMMOB_MAP_NET pOb)
{
    LocalFree(pOb->wszMultiText);
}

/*
* Convert the "old soon to be replaced" network connection format to a "map" format.
*/
PVMMOB_MAP_NET VmmWinNet_Initialize_DoWork()
{
    LPCWSTR wszSTATES[] = {
        L"CLOSED",
        L"LISTENING",
    ...
    DWORD i, cTcpIp, cchMultiText;
    PVMMWIN_TCPIP_ENTRY pn, pTcpIp = NULL;
    PVMMOB_MAP_NET pObNet = NULL;
    PVMM_MAP_NETENTRY pe;
    DWORD dwIpVersion, cwszSrc, cwszDst;
    WCHAR wszSrc[64], wszDst[64];
    if(!VmmWinTcpIp_TcpE_Get(&pTcpIp, &cTcpIp)) { goto fail;}
    if(!(pObNet = Ob_Alloc(OB_TAG_MAP_NET, LMEM_ZEROINIT, sizeof(VMMOB_MAP_NET) + cTcpIp * sizeof(VMM_MAP_NETENTRY), VmmWinNet_CallbackCleanup_ObMapNet, NULL))) { goto fail; }
    ...
    for(i = 0; i < cTcpIp; i++) {
        ...
        // general
        pe->dwPID = pn->dwPID;
        pe->dwState = min(10, pn->dwState);
        pe->AF = pn->AF.fValid ? pn->AF.wAF : 0;
        pe->vaObj = pn->vaTcpE;
        pe->ftTime = pn->qwTime;
        // src
        pe->Src.fValid = pn->Src.fValid;
        pe->Src.port = pn->Src.wPort;
        memcpy(pe->Src.pbAddr, pn->Src.pbA, 16);
        if(pn->Src.fValid && (pe->Src.wszText = (LPWSTR)InetNtopW(pe->AF, pe->Src.pbAddr, pObNet->wszMultiText + cchMultiText, 64))) {
            cchMultiText += (DWORD)wcslen(pObNet->wszMultiText + cchMultiText) + 1;
        }
        if(!pe->Src.wszText) { pe->Src.wszText = pObNet->wszMultiText + 1; }
        // dst
        pe->Dst.fValid = pn->Dst.fValid;
        pe->Dst.port = pn->Dst.wPort;
        memcpy(pe->Dst.pbAddr, pn->Dst.pbA, 16);
        if(pn->Dst.fValid && (pe->Dst.wszText = (LPWSTR)InetNtopW(pe->AF, pe->Dst.pbAddr, pObNet->wszMultiText + cchMultiText, 64))) {
            cchMultiText += (DWORD)wcslen(pObNet->wszMultiText + cchMultiText) + 1;
        }
        ...
        cwszDst = _snwprintf_s(wszDst, _countof(wszDst), _TRUNCATE, ((dwIpVersion == 6) ? L"[%s]:%i" : L"%s:%i"), pe->Dst.wszText, pe->Dst.port);
...
PVMMOB_MAP_NET VmmWinNet_Initialize()
{
    ...
    pObNet = VmmWinNet_Initialize_DoWork();
    if(!pObNet) {
