...
VOID MHeap_HeapReadLineCB(_In_ PMHEAP_CTX ctx, _In_ DWORD cbLineLength, _In_ DWORD ie, _In_ PVMM_MAP_HEAPENTRY pe, _Out_writes_(cbLineLength + 1) LPSTR szu8)
{
    Util_usnprintf_ln(szu8, cbLineLength,
        ...
        (!ctxVmm->f32 && pe->f32) ? " (32)" : ""
    );
}

VOID MHeap_SegmentReadLineCB(_In_ PMHEAP_CTX ctx, _In_ DWORD cbLineLength, _In_ DWORD ie, _In_ PVMM_MAP_HEAP_SEGMENTENTRY pe, _Out_writes_(cbLineLength + 1) LPSTR szu8)
{
    Util_usnprintf_ln(szu8, cbLineLength,
        "%04x%7i %4i %016llx %s",
        ie,
...
VOID MHeap_AllocReadLineCB(_In_ PMHEAP_CTX ctx, _In_ DWORD cbLineLength, _In_ DWORD ie, _In_ PVMM_MAP_HEAPALLOCENTRY pe, _Out_writes_(cbLineLength + 1) LPSTR szu8)
{
    BYTE pb16[16] = { 0 };
    CHAR szHex[80] = { 0 };
    DWORD cbHex;
    if(ctx->fVerbose) {
        cbHex = sizeof(szHex);
        VmmRead(ctx->pProcess, pe->va, pb16, min(16, pe->cb));
...
_Success_(return)
BOOL MHeap_GetAllocPath(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Out_ PVMMOB_MAP_HEAPALLOC *ppObHeapAllocMap, _Out_ LPSTR *pszPath)
{
    ...
    *pszPath = CharUtil_PathSplitNext(ctxP->uszPath);
    return VmmMap_GetHeapAlloc(ctxP->pProcess, dwId, ppObHeapAllocMap);
}
...
* -- return
*/
NTSTATUS MHeap_Write(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _In_reads_(cb) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbWrite, _In_ QWORD cbOffset)
{
    QWORD va;
    LPSTR uszPath;
    ...
    *pcbWrite = 0;
    if(!MHeap_GetAllocPath(ctxP, &pObHeapAllocMap, &uszPath)) { goto finish; }
    if(CharUtil_StrEndsWith(uszPath, ".mem", FALSE)) {
        uszPath = CharUtil_PathSplitLast(uszPath);
        va = Util_GetNumericA(uszPath);
        if(va && (peA = VmmMap_GetHeapAllocEntry(pObHeapAllocMap, va))) {
            VmmWriteAsFile(ctxP->pProcess, peA->va, peA->cb, pb, cb, pcbWrite, cbOffset);
        }
        goto finish;
    }
finish:
...
*/
_Success_(return == 0)
NTSTATUS MHeap_Read(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset)
{
    ...
    LPSTR uszPath;
    QWORD va;
    if(!VmmMap_GetHeap(ctxP->pProcess, &pObHeapMap)) { return VMMDLL_STATUS_FILE_INVALID; }
    ...
    if(!_stricmp(ctxP->uszPath, "heaps.txt")) {
        nt = Util_VfsLineFixed_Read(
            (UTIL_VFSLINEFIXED_PFN_CB)MHeap_HeapReadLineCB, &ctx, MHEAP_HEAP_LINELENGTH, MHEAP_HEAP_LINEHEADER,
    ...
    if(!_stricmp(ctxP->uszPath, "segments.txt")) {
        nt = Util_VfsLineFixed_Read(
            (UTIL_VFSLINEFIXED_PFN_CB)MHeap_SegmentReadLineCB, &ctx, MHEAP_SEGMENT_LINELENGTH, MHEAP_SEGMENT_LINEHEADER,
    ...
    if(!_stricmp(uszPath, "allocations.txt")) {
        nt = Util_VfsLineFixed_Read(
            (UTIL_VFSLINEFIXED_PFN_CB)MHeap_AllocReadLineCB, &ctx, MHEAP_ALLOC_LINELENGTH, MHEAP_ALLOC_LINEHEADER,
    ...
    if(!_stricmp(uszPath, "allocations-v.txt")) {
        ctx.fVerbose = TRUE;
        nt = Util_VfsLineFixed_Read(
            (UTIL_VFSLINEFIXED_PFN_CB)MHeap_AllocReadLineCB, &ctx, MHEAP_ALLOCV_LINELENGTH, MHEAP_ALLOCV_LINEHEADER,
            pObHeapAllocMap->pMap, pObHeapAllocMap->cMap, sizeof(VMM_MAP_HEAPALLOCENTRY),
            pb, cb, pcbRead, cbOffset
        );
        goto finish;
    ...
    if(CharUtil_StrEndsWith(uszPath, ".mem", FALSE)) {
        ...
        va = Util_GetNumericA(uszPath);
        if(va && (peA = VmmMap_GetHeapAllocEntry(pObHeapAllocMap, va))) {
            nt = Util_VfsReadFile_FromMEM(ctxP->pProcess, va, peA->cb, 0, pb, cb, pcbRead, cbOffset);
        }
        goto finish;
    }
finish:
...
* -- return
*/
BOOL MHeap_List(_In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Inout_ PHANDLE pFileList)
{
    PVMMOB_MAP_HEAP pObHeapMap = NULL;
    PVMMOB_MAP_HEAPALLOC pObHeapAllocMap = NULL;
    ...
    LPSTR uszPath;
    DWORD i, iBase;
    LPSTR szFORMATMEM = ctxVmm->f32 ? "0x%08llx.mem" : "0x%012llx.mem";
    if(!VmmMap_GetHeap(ctxP->pProcess, &pObHeapMap)) { goto finish; }
    ...
    if(!ctxP->uszPath[0]) {
        ...
        VMMDLL_VfsList_AddFile(pFileList, "heaps.txt", UTIL_VFSLINEFIXED_LINECOUNT(pObHeapMap->cMap) * MHEAP_HEAP_LINELENGTH, NULL);
        VMMDLL_VfsList_AddFile(pFileList, "segments.txt", UTIL_VFSLINEFIXED_LINECOUNT(pObHeapMap->cSegments) * MHEAP_SEGMENT_LINELENGTH, NULL);
        goto finish;
    }
    // specific heap
    if(!MHeap_GetAllocPath(ctxP, &pObHeapAllocMap, &uszPath)) { goto finish; }
    if(!uszPath[0]) {
        ...
        VMMDLL_VfsList_AddFile(pFileList, "allocations.txt", UTIL_VFSLINEFIXED_LINECOUNT(pObHeapAllocMap->cMap) * MHEAP_ALLOC_LINELENGTH, NULL);
        VMMDLL_VfsList_AddFile(pFileList, "allocations-v.txt", UTIL_VFSLINEFIXED_LINECOUNT(pObHeapAllocMap->cMap) * MHEAP_ALLOCV_LINELENGTH, NULL);
        goto finish;
    }
...
* -- pPluginRegInfo
*/
VOID M_ProcHeap_Initialize(_Inout_ PVMMDLL_PLUGIN_REGINFO pRI)
{
