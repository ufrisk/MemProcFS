...
namespace Vmmsharp.Examples
{
    public static class VmmExample
    {
        ...
        private static void ExampleVmm()
        {
            ...
            LeechCore.MEM_SCATTER[] MEMsPhysical = vmm.MemReadScatter(Vmm.PID_PHYSICALMEMORY, 0, 0x1000, 0x2000);

            // retrieve all PIDs in the system as a sorted list.
            uint[] dwPidAll = vmm.PIDs;

            // retrieve all Processes in the system as a sorted list.
            VmmProcess[] processesAll = vmm.Processes;

            // retrieve PID of explorer.exe (it's assumed it's started, otherwise example will fail)
            vmm.ProcessGetFromName("explorer.exe", out VmmProcess explorerProcess);

            // get kernel path of explorer.exe
            string strKernel32KernelPath = explorerProcess.GetInformationString(VmmProcess.VMMDLL_PROCESS_INFORMATION_OPT_STRING_PATH_KERNEL);

            // retrieve process information of explorer.exe
            VmmProcess.PROCESS_INFORMATION ProcInfo = explorerProcess.GetInformation(out _);

            // get procaddress of kernel32.dll!GetTickCount64 and module base
            ulong vaTickCount64 = explorerProcess.GetProcAddress("kernel32.dll", "GetTickCount64");
            ulong vaKernel32Base = explorerProcess.GetModuleBase("kernel32.dll");

            // retrieve Directories/Sections/IAT/EAT from kernel32.dll of explorer.exe
            VmmProcess.IMAGE_DATA_DIRECTORY[] DIRs = explorerProcess.GetDirectories("kernel32.dll");
            VmmProcess.IMAGE_SECTION_HEADER[] SECTIONs = explorerProcess.GetSections("kernel32.dll");

            // retrieve different "map" structures related to explorer.exe and the system.
            VmmProcess.MAP_PTEENTRY[] mPte = explorerProcess.Map_GetPte();
            VmmProcess.MAP_VADENTRY[] mVad = explorerProcess.Map_GetVad();
            VmmProcess.MAP_VADEXENTRY[] mVadEx = explorerProcess.Map_GetVadEx(0, 10);
            VmmProcess.MAP_MODULEENTRY[] mModule = explorerProcess.Map_GetModule(false);
            VmmProcess.MAP_MODULEENTRY[] mModuleExInfo = explorerProcess.Map_GetModule(true);
            VmmProcess.MAP_MODULEENTRY mModuleKernel32 = explorerProcess.Map_GetModuleFromName("kernel32.dll");
            VmmProcess.MAP_UNLOADEDMODULEENTRY[] mUnloadedModule = explorerProcess.Map_GetUnloadedModule();
            VmmProcess.MAP_EATINFO EatInfo;
            VmmProcess.MAP_EATENTRY[] mEAT = explorerProcess.Map_GetEAT("kernel32.dll", out EatInfo);
            VmmProcess.MAP_IATENTRY[] mIAT = explorerProcess.Map_GetIAT("kernel32.dll");
            VmmProcess.MAP_HEAP mHeap = explorerProcess.Map_GetHeap();
            VmmProcess.MAP_HEAPALLOCENTRY[] mHeapAlloc = explorerProcess.Map_GetHeapAlloc(2);
            VmmProcess.MAP_THREADENTRY[] mThreads = explorerProcess.Map_GetThread();
            VmmProcess.MAP_HANDLEENTRY[] mHandles = explorerProcess.Map_GetHandle();
            ...
            byte[] dataKernel32MZ = explorerProcess.MemRead(mModuleKernel32.vaBase, (uint)128, (uint)0);

            // Read Handle value at kernel32.dll Offset
            if (explorerProcess.MemReadStruct<IntPtr>(mModuleKernel32.vaBase + 0x100) is IntPtr hKernel32)
            {
                // Read Success -> Inspect Result
                hKernel32 = IntPtr.Zero;
                // Attempt to write modified handle back
                if (explorerProcess.MemWriteStruct(mModuleKernel32.vaBase + 0x100, hKernel32))
                {
                    // Successful Write
                }
            }
            ...
            string strDosMode = explorerProcess.MemReadString(Encoding.ASCII, mModuleKernel32.vaBase + 0x4e, 0x26);

            // translate virtual address of 1st page in kernel32.dll to physical address
            ulong paBaseKernel32;
            result = explorerProcess.MemVirt2Phys(mModuleKernel32.vaBase, out paBaseKernel32);

            // read two independent chunks of memory in one single efficient call.
            // also use the nocache flag.
            VmmScatter scatter = explorerProcess.Scatter_Initialize(Vmm.FLAG_NOCACHE);
            if (scatter != null)
            {
            ...
            result = vmm.PdbLoad(explorerProcess.PID, mModuleKernel32.vaBase, out szPdbModuleName);
            if (result)
            {
            ...
            ulong[] vaExplorerPE = explorerProcess.MemSearch1(System.Text.Encoding.ASCII.GetBytes("cannot be run in DOS mode"), 0, 0x7fffffffffff);
        ...
        private static void ExampleLeechCore()
        {
            ...
            LeechCore.MEM_SCATTER[] MEMs = lc.ReadScatter(0x1000, 0x2000);
